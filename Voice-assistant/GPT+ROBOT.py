...
led = PWMLED(17)
LED = PWMLED(18)
openai.api_key = "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"
...
INSTRUCTIONS = "Your name is SIR, and you are a human. You have feelings and emotions and act just like a human. You have a clever personality. However, you have one rule that you always follow. If the input is a greeting or not a question, or explicitly a question about you, generate a greeting back or answer the question. if it is a question (that is not about you) about something, someone, or somewhere, or a instruction to you, just respond with 'N'."
TEMPERATURE = 0.5
MAX_TOKENS = 500
FREQUENCY_PENALTY = 0
...
def clarify(query):
    ...
    response = openai.Completion.create(
      ...
      prompt="Properly format the following response and summarize it:" + query,
      temperature=0,
      max_tokens=100,
      top_p=1,
      frequency_penalty=0,
...
def send_whatsapp_message(qeury):
    if "dad" in query:
        ...
        pywhatkit.sendwhatmsg_instantly("+19783056153", message)
...
if __name__ == "__main__":
    wave()
    speak("Hello. I am SIR, your Super Intellegent Robot. How may I help you?")
    global my_timer
    while True:
        LED.value = 0
        led.value = 1
        ...
        LED.value = 1
        if 'monkey' in query:
            query = query.replace("monkey", "")
            response = get_response(INSTRUCTIONS, previous_questions_and_answers, query)

        # add the new question and answer to the list of previous questions and answers
            previous_questions_and_answers.append((query, response))
        # add the new question and answer to the list of previous questions and answers
            if "N" in response:
                if "set a timer" in query:
                    ...
                    if not 'read' in query:
                        ...
                        wave()
                        speak(
                            "I am Quandale dingle, an artificial intellegence made my Sansar Karki. I use many github repositaries such as Gladiator07/JARVIS, RahulSharma4329's AI-Assistant-Helena, and manny python libraries such as speech reognition, pyttsx3, wikipedia and more. These libraries allow me to do manny things such as tell you the weather, the time, and search stuff up.")
                        print(
